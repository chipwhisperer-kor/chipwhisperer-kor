...
수집 노트북(`0.0`/`0.1`)은 하드웨어가 있을 때만 돌아간다. `Bench.capture()`는 장시간
수집 중 실패하면 캡처 재시도, USB 재연결, Husky 펌웨어 재기록 순으로 복구하고 측정
설정과 마스크 시드를 복원한다. 복구 단계와 중단 조건은 §8에 정리되어 있다.
...
수집 직후 자동으로 호출되며 빈 목록이면 그 파일이 스키마 검증을 통과한 것이다. 생성된
HDF5 파일은 용량 때문에 저장소에 포함하지 않으므로, 현재 체크아웃만으로 두 Dataset의
준수 상태를 주장하지 않는다. 각 수집 뒤 실제 파일을 다시 검증해야 한다.

### 수집 목표 Trace 수

| Subset | 목표 Trace 수(두 타겟 동일) |
|------|------|
| explore | 5,000 |
| profiling | 100,000 |
| attack | 10,000 |
