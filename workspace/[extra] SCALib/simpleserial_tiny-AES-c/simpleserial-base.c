...
/* SimpleSerial 1.1 콜백은 빌드 호환성만 유지한다. 수집기의 키·평문·길이 프로토콜은
...
/* 0x81 입력 명령: 'k'는 AES 키, 'p'는 평문을 저장하고 같은 바이트를 응답한다. 'l'은
...
/* 0x82 'c' 명령: 트리거가 HIGH인 동안에만 AES 연산을 수행한다. 따라서 수집된
...
/* 0x83 'r' 명령: 직전 AES 결과를 'l'로 지정한 길이만큼 반환한다. 이 함수는 결과를
