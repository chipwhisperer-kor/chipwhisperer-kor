...
def main() -> None:
    ...
    if args.source.stem != args.output.stem:
        raise SystemExit("Source and output must use the same stem.")

    source_rel = args.source.as_posix()
    if source_rel.startswith("/") or args.output.as_posix().startswith("/"):
        raise SystemExit("Use project-root-relative paths.")
