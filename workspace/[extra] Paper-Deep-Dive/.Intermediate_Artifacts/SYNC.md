...
## 사본 목록 (2026-07-24 — 활성 **1개**)

| 사본 | 절대경로 | 상태 |
|------|----------|------|
| **작업 루트** | `/home/user/chipwhisperer-kor/workspace/[extra] Paper-Deep-Dive` | **유일한 활성 사본** |

**사본 단일화 달성** (D54). 휴지통에 아래가 있으나 활성 사본이 아니다 — 복원하지 말 것.

| 휴지통 항목 | 내용 |
|-------------|------|
| `~/.local/share/Trash/files/fia_cm_haetae/` | 구 동결 백업(`Collabo_HB/Paper-Deep-Dive`) 포함. 사용자가 이동 |
| `~/.local/share/Trash/files/[extra] Paper-Deep-Dive/` | 2026-07-22 · 폐지된 `To_Do.md`·`README_PATH.md` 잔존 |
| `~/.local/share/Trash/files/.Prompt_Engineering/` | 폐기된 설계 초안 |

삭제 이력: Grok worktree 2종은 **D50·D52**로 제거, `fia_cm_haetae` 백업은 사용자가 휴지통으로 이동(**D54** 확인).
...
| 시각 | 내용 |
|------|------|
| 2026-07-24 | **D54** 백업이 휴지통으로 이동된 것을 `초기화`가 적발 → **활성 사본 1개로 단일화 완료**. 목록 정정 |
