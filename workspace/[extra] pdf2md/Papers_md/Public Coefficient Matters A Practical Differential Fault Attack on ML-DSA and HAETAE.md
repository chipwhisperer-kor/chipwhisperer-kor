...
In this paper, every element P is defined over Z q , and poly- nomials take the form z = i z[i]X i , where each coefficient z[i] ∈ Z q denotes the i-th component of z. We consider

2

the ring R q = Z q [X]/(X n + 1), and polynomial vectors are defined over R q ℓ or R q k , where L and k denote the dimensions of the corresponding vector space. The vectors and matrices are denoted as lower and upper bold letters, respectively y, A. Vectors and matrices in the NTT domain are denoted by ŷ and Â. The multiplication in the NTT domain is represented as x̂ · ŷ, whereas polynomial multiplication in the standard domain is denoted by x ∗ y. To discriminate between genuine and faulted values, the faulted counterpart of a polynomial vector z is denoted as z ′ . This notation is used for both the intended and unintended faults. The difference between two values z and z ′ can then be expressed as ∆z.
...
Algorithm 1 ML-DSA Sign Input: Message m and sk = (ρ, tr, t 0 , s 1 , s 2 , key) Output: Signature σ 1: µ ← H(tr, m) 2: Construct Matrix A with ρ 3: ρ ′ ← H(key, µ) 4: ctr ← 0 5: y ← ExpandY(ρ ′ , ctr) 6: (w 0 , w 1 ) ← InvNTT(A · ŷ) 7: c̃ ← H(w 1 , µ) 8: c ← SampleChallenge(c̃) 9: z ← y + InvNTT(ĉ · s ˆ 1 ) 10: r ← w 0 − InvNTT(ĉ · s ˆ 2 ) 11: if z, r satisfy norm bounds γ 1 − β, γ 2 − β then 12: if c ∗ t 0 satisfies norm bound γ 2 then 13: h ← MakeHint(r, c ∗ t 0 ) 14: return σ = (c̃, z, h) 15: else 16: ctr ← ctr + l and Repeat from Line 5 17: end if 18: else 19: ctr ← ctr + l and Repeat from Line 5 20: end if

pk, secret vectors s 1 and s 2 , and key. Note that, key is only required for deterministic signing.

Algorithm 2 HAETAE KeyGen Output: Public Key pk, Secret Key sk 1: seed ← {0, 1} ρ 2: seed A , seed sk , K ← H(seed) 3: A ← expandA(seed A ) 4: ctr ← 0 5: repeat 6: s 1 , s 2 ← expandS(seed sk , ctr) 7: ctr ← ctr + (l − 1) + k 8: (t 0 , t 1 ) ← A · s 1 + s 2 9: s 2 ← s 2 − t 0 10: until s 1 and s 2 in bound 11: return pk = (seed A , t 1 ), sk = (pk, s 1 , s 2 , K)
...
Algorithm 3 HAETAE Sign Input: Message m and sk = (pk, s 1 , s 2 , key) Output: Signature σ 1: µ ← H(seed A , t 0 , m) 2: Construct Matrix A with ρ 3: ρ ′ ← H(key, µ) 4: ctr ← 0 5: (y 1 , y 2 , b, b ′ ) ← ExpandY(ρ ′ , ctr) 6: w ← InvNTT( Â · y ˆ 1 ) + 2 ∗ y 2 7: c ← SampleChallenge(w, µ) 8: z 1 ← y 1 + (−1) b ∗ InvNTT(ĉ · s ˆ 1 ) 9: z 2 ← y 2 + (−1) b ∗ InvNTT(ĉ · s ˆ 2 ) 10: h ← MakeHint(w, z 2 ) 11: if ||z|| 2 ≥ B ′ or ||2z − y|| 2 &lt; B ∧ b ′ then 12: ctr++ and Repeat from Line 5 13: else 14: return σ = (c, z 1 , h) 15: end if
...
Algorithm 4 Challenge Sampling for HAETAE-120 &amp; 180 Input: Seed w and Seed M Output: Challenge c 1: state ← H(w, M ) 2: Initialize c with zero 3: for i ← 255 − τ to 255 do 4: while update &gt; i do 5: if pos ≥ len then 6: state ← N B(state, 1) 7: pos ← 0 8: end if 9: update ← state[pos] 10: pos ← pos + 1 11: end while 12: c[i] ← c[update] 13: c[update] ← 1 14: end for 15: return c

Algorithm 5 Challenge Sampling for HAETAE-260 Input: Seed w and Seed M Output: Challenge c 1: state ← H(w, M ) 2: mask ← 0 3: if HW (state) &gt; 128 then 4: mask ← −1 5: end if 6: if HW (state) == 128 then 7: mask ← −(state[0] ∧ 1) 8: end if 9: for i ← 0 to 31 do 10: state[i] ← state[i] ⊕ mask 11: for n ← 0 to 7 do 12: c[8 ∗ i + n] ← (state[i] &gt;&gt; n) ∧ 1 13: end for 14: end for 15: return c
...
Algorithm 6 Challenge Sampling for ML-DSA Input: State state Output: Challenge c 1: sign ← 0 2: update ← 0 3: for i ← 0 to 8 do 4: sign ← sign ∨ (N B(state, 1) ≪ 8 ∗ i) 5: end for 6: Initialize c with zero 7: for i ← 255 − τ to 255 do 8: while update &gt; i do 9: if pos ≥ len then 10: state ← N B(state, 1) 11: pos ← 0 12: end if 13: update ← state[pos] 14: pos ← pos + 1 15: end while 16: c[i] ← c[update] 17: c[update] ← 1 − 2 ∗ (sign ∧ 1) 18: sign ← sign ≫ 1 19: end for 20: return c
...
1. Initialize Counter i 2. if(i == END) 3. Loop Abort 4. Execute Loop Body 5. i ← i + 1 6. Return to Line 2

Fig. 1. Loop structure targeted by the proposed attack.
...
sc[i] = c[j] ∗ s 1 [i − j] − c[j] ∗ s 1 [n + i − j]. (1)
...
Algorithm 7 DFA on HAETAE and ML-DSA Input: Normal Signature σ = (c, z 1 ) and Faulty Signature σ ′ = (c ′ , z 1 ′ ) Output: Polynomial s 1: sc ← z − z ′ 2: loc ← (c − c ′ ) −1 3: x ← Multiply sc with loc 4: return s
...
⓿ Inject Fault Utilizing Fault Parameters

❷ Recovering Absolute Value of Secret Key

Deterministic HAETAE Sign
...
Fig. 2. Framework of recovering secret keys s 1 and s 2 of HAETAE.
...
Algorithm 8 Recovery of b

Input: Recovered Partial Secret Keys (−1) b ∗s 1 and (−1) b ∗ s 2 , Public Key pk = (seed A , pk b 1 ) Output: b recoverd 1: A ← ExpandA(seed A ) 2: guess s 1 ← (−1) b ∗ s 1 3: guess s 2 ← (−1) b ∗ s 2 4: b ′ ← A · guess s 1 + guess s 2 5: b ′ guess ← Decompose(b ′ ) 6: if Compare b ′ guess and pk b 1 then 7: return 0 8: else 9: return 1 10: end if
...
❸ Recovering Full Secret Key for Signature Forgery
...
⓿ Without Fault Injection (Pre-processing)

Deterministic ML-DSA Sign
...
Fig. 3. Framework of recovering secret keys of ML-DSA.
...
TABLE I I MPACT OF THE P ROPOSED A TTACK ON R EJECTION S AMPLING I TERATIONS IN LATEST (V ERSION 3.0 FOR HAETAE AND FIPS 204 FOR ML-DSA) AND PQM 4 IMPLEMENTATION

Match

Less

Implementation latest pqm4 latest pqm4
...
TABLE II S ECRET K EY R ECOVERY R ESULTS AND R EMAINING G UESSING C OMPLEXITY

Implementation Required Faulty Signature Recover Rate Guess

HAETAE (Our Work) ML-DSA (Our Work) ML-DSA [13] ML-DSA-65 [17] ML-DSA-44 [19]

1/1/1 1/1/1 7.3 / 13 / 15.8 13 1
...
TABLE III C OMPARISON OF F AULT -I NDUCED S IGNATURES L EADING TO S UCCESSFUL R ECOVERY
...
Algorithm 9 HAETAE2 Countermeasure Sign Input: Message M , Secret Key sk = (seed A , b1, s 1 , s 2 , K) Output: Signature σ 1: µ ← H(seed A , b 1 , M ) 2: Construct Matrix A with seed A 3: seed y ← H(K, µ) 4: ctr ← 0 5: (y 1 , y 2 , b, b ′ ) ← ExpandY(seed y , ctr) 6: w ← A · ⌊y⌉ 7: c, iter ← SampleChallenge(w, µ) 8: if iter ! = N then 9: Repeat from Line 5 10: end if 11: z 1 ← y 1 + (−1) b ∗ (ĉ · s ˆ 1 ) 12: z 2 ← y 2 + (−1) b ∗ (ĉ · s ˆ 2 ) 13: h ← MakeHint(w, z 2 ) 14: if ||z|| 2 ≥ B ′ or ||2z − y|| 2 &lt; B ∧ b ′ then 15: ctr++ and Repeat from Line 5 16: else 17: return σ = (c, z 1 , h) 18: end if
