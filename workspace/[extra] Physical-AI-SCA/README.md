...
| 구현 (소스 → 기계어) | 전이 누설, share 재사용, 컴파일러가 만든 스필 | **에뮬레이션** | ✅ 실행·검증됨 |
...
**실물에서 안 보여도 에뮬레이션에서는 보인다** — 실측은 잡음에 묻힌다.
...
잡음에 묻히고, 명령어 단위로 짚을 수 없다. 에뮬레이션은 정반대 조건을 준다:
**잡음 0, 샘플 하나가 명령어 하나.** 그래서 종속성이 있으면 무조건 보이고, 보이는 즉시
**어느 명령어인지 지목**할 수 있다.
...
# (참고) 아무 데이터셋에나 요건 대조표만 돌려 볼 수도 있다
...
| 문서 | 생성 시점 | 담는 것 |
|---|---|---|
| `01_experiment_plan.md` | **수집 전** | 적용 범위 선언(`not_claimed` 포함) · 요건 대조표 · 판정 기준 · 장수 산정 근거 · 벤더 정보 |
...
| **SPA** | key derivation (§8.3.1) | A.2.2 — 11 파형, 육안 + 통계 **둘 다**. 도구는 통계만 하므로 **판정을 내지 않는다**(§9) |
...
| 대조군 | 기대 | 어긋나면 |
|---|---|---|
| `tiny-AES-c` (비마스킹) | 도처에서 검출 | **검출기가 고장난 것. 진행 중단** |
...
교차 검증: 실측 h5 의 샘플 수 비율(masked/normal)이 **1.6636**, 에뮬레이션 명령어 수
비율이 **1.6828** (10,147 / 6,030) 이다. 1.2 % 차이는 명령어별 사이클 수가 다르기
때문이다 — 실측은 사이클을, 에뮬은 명령어를 센다.
...
| **SPA 판정 불가** | 육안 검사가 사람의 행위이고, 잡음 바닥이 0 인 결정적 채널에서는 "키가 다르면 파형도 다르다" 가 거의 항상 참이라 그것만으로 fail 을 내면 판별력이 없다. **언제나 `inconclusive`** 를 내고 관측값(`statistical_verdict`)으로 사람이 판정한다 |
| **STM32F303 캐시 유무 미확인** | §8.2 면제의 전제인데 저장소 안에 근거 문서가 없다. 대조표에 `미기록` 으로 나온다 |
| **EM 채널 없음** | 근접 자기장 프로브 미보유. Annex E 는 오히려 EM 을 선호한다고 적는다 |
| **장수 부족** | 데모 규모라 Formula (1) 의 N 에 못 미친다. DPA 는 `inconclusive` 로 보고된다 |

---

> 위 한계를 해소하려면 무엇을 해야 하는지는 **[`To_Do_List.md`](To_Do_List.md)** 에
> 항목별로 적어 두었다 — 실물 하드웨어가 있어야 하는 것과 지금 확인할 수 있는 것을
> 나누고, 각각에 완료 판정 기준을 달았다.
...
physai/
  ...
  collect.py       CLI: spec → SCHEMA 1.1 준수 h5
  collectors/
    emulation.py   ★ 에뮬레이션 수집기 (실행·검증됨)
...
| `workspace/iut/` | 검증 대상 암호 라이브러리 (펌웨어와 공유) |
