...
<!-- _class: lead -->
# ChipWhisperer 기반<br>부채널 분석 전략

## <center>전력 파형 수집부터 와이어태핑까지</center>
...
암호 알고리즘이 **수학적으로 안전**하더라도, 하드웨어가 연산을 수행할 때 새어 나오는 **부수 정보**(전력 소비·전자기 방사·처리 시간)에는 비밀 키와 관련된 정보가 누설

이를 측정·분석해 키를 복원하는 기법이 **부채널 분석(Side-Channel Analysis)**
...
**ChipWhisperer**는 오실로스코프와 정밀 트리거 환경을 한 보드에 통합한 오픈소스 플랫폼
...
목표: **"전력 파형을 안정적으로 수집하기"**

| 단계 | 내용 | 핵심 |
| :----: | :---- | :---- |
| **1** | ChipWhisperer 타겟보드  데이터 송수신 | SimpleSerial 통신 |
| **2** | 파형 수집 파라미터 이해 | `samples` 결정 |
| **3** | 단일 파형 수집 및 시각화 | 1개의 Power Trace 확인 |
| **4** | 다량 파형 수집 + DB 저장 | `*.h5` 파일 |
| **5** | HDF5 DB 불러오기 및 검증 | 저장 데이터 재확인 + 시각화 |

> 측정 및 분석 결과의 신뢰성은 곧 **파형 수집의 품질**

---

## 1단계 — SimpleSerial 패킷 구조

ChipWhisperer는 **SimpleSerial 프로토콜**로 타겟과 통신

```text
SimpleSerial 프로토콜 패킷 구조
┌──────┬──────┬─────────┬──────────┬─────┐
...
> SimpleSerial V1은 패킷당 최대 64바이트, **V2는 최대 249바이트**를 전송

---

## 1단계 — 골든 모델(Golden Model) 검증

예제에서 타겟은 단순한 `k XOR p` 연산을 수행
호스트가 직접 계산한 값(골든 모델)과 비교해 **통신·연산의 정상성**을 검증
...
> 💡 골든 모델과 하드웨어 출력이 일치하면 **펌웨어와 통신 모두 정상**임을 의미

---

## 2단계 — 파형 수집 파라미터

타겟 연산이 몇 클럭이나 걸리는지 측정해, **잘리지 않으면서도 메모리 낭비가 없는** 적정 `samples`를 결정
...
ChipWhisperer-Husky는 타겟 MCU 클럭에 **동기화된 ADC**를 사용
...
MCU가 7.37 MHz로 동작하면 ADC는 **29.5 MHz**로 샘플링 → 파형이 더 정밀
...
`(타겟 출력값, 파형 numpy 배열)` 튜플을 반환

**Bokeh**(인터랙티브) : 파형 시각화 추천 도구
...
부채널 분석에는 보통 **수천~수만 개의 파형**이 필요
메모리에 한꺼번에 올리지 않고 **HDF5 데이터셋에 점진적으로 append**
...
> 본격 수집 전 점검용 모드(`debug_mode`)로 대량 수집 전에 수집코드의 유효성 판단

---

## 5단계 — DB 불러오기 & 누설 확인

저장한 `.h5`를 다시 불러와 구조를 확인하고, `o == i_k XOR i_p` 관계를 검증
여러 파형을 **겹쳐(overlay)** 그려 수집 안정성을 점검
...
단일 보드 실습에서는 측정 장비가 곧 **"공격자가 통제하는 통신 단말"**
☞ 실제 공격자는 보통 **동작 중인 시스템의 신호선에 프로브만 부착**, 통신 흐름에는 개입이 불가능

* 상기한 **수동 측정(passive measurement) 시나리오**를 두 장치로 재현
...
|:----:|:----:|:----:|
| 측정 장비 수 | 1대 (Husky) | **2대 (Lite_대상장비 + Husky_공격자)** |
| UART 통신 / 프로그래밍 | Husky | **Lite** 전담 |
| 타겟 클럭 공급원 | Husky `clkgen` | **Lite `clkgen` (HS2)** |
| 측정 장치 역할 | 통신 + 측정 | 공격자는 **측정만** (passive observer) |
...
> 실제 공격에서는 통신 신호(UART idle, 특정 패턴 등)를 트리거로 활용
...
| **2** | Lite와 타겟보드 통신 채널 확보 | `target` 객체 |
...
`cw.scope()`만 호출하면 **가장 먼저 발견된 장치 하나**만 연결
☞ 두 장치를 동시에 다루려면 **시리얼 넘버**를 명시
...
와이어태핑 시나리오에서 Husky는 **외부 클럭을 받아오는 입장**
☞ 주파수·위상을 모르므로 **탐색·정렬** 필요
...
두 스코프를 **모두 arm한 뒤** 단일 트리거가 발생해야 두 파형이 시간축으로 정렬
...
> `Encrypt()` *후* 에 `arm()`을 부르면 트리거가 이미 지나가 캡처가 실패

---

## 7단계 — Husky vs Lite 파형 비교

동일 연산을 두 스코프가 캡처한 파형을 **Bokeh으로 겹쳐** 와이어태핑 경로의 충실도를 검증
...
| 2 | `cw.target(lite_scope, SimpleSerial2)` | Lite 타겟보드 채널 확립 |
...
와이어태핑 부채널 분석을 위한 **파형 수집 파이프라인**을 두 단계

1. **1부** — 단일 장치로 SimpleSerial 통신부터 HDF5 DB 저장까지, *안정적 trace 수집*의 기본 수립
